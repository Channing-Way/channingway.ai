...
## Versioning: pin third-party, drift first-party
...
Pin third-party dependencies. Each pinned ecosystem needs both a dependabot-parseable manifest location for the pin AND a corresponding entry in [`.github/dependabot.yml`](dependabot.yml) so drift is automated rather than manual. Introducing a new pinned dependency class means placing the pin somewhere dependabot's ecosystem can parse (or explicitly documenting it as manually maintained) plus adding the ecosystem entry in the same PR.

- **GitHub Actions** — pin to a 40-character commit SHA with the semver tag in a trailing comment (`uses: org/action@<sha>  # v<tag>`). Matches [GitHub's supply-chain hardening guidance](https://docs.github.com/en/actions/security-guides/security-hardening-for-github-actions#using-third-party-actions) and defends against tag-swap attacks. Automated drift: dependabot `github-actions` ecosystem parses `.github/workflows/*.yml`, monthly cadence (current config).
- **npm build dependencies** — pin to an exact semver (`pkg@X.Y.Z`), not `@latest` or `@^X`. Applies to any package installed globally or from source during the build (for example, `wrangler` inside `Dockerfile`). Automated drift: dependabot's `npm` ecosystem parses `package.json` / `package-lock.json`; it does **not** rewrite versions embedded in `RUN npm install pkg@X.Y.Z` lines inside a Dockerfile. To get automated drift on a build-time npm pin, place the pin in a dependabot-native manifest (add a small build-scoped `package.json` and have the Dockerfile `COPY package.json ./ && npm install`) or adopt an updater that rewrites Dockerfile RUN commands (Renovate supports this; dependabot does not). Where neither refactor is warranted, document the pin as manually maintained with a scheduled review cadence rather than implying dependabot coverage.
- **Container base images** — pin to a specific slim tag (`node:22-slim`) at minimum; prefer a digest pin (`node:22-slim@sha256:...`) where the downstream will re-pull the image on every build. Never `:latest`. Automated drift: dependabot's `docker` ecosystem parses `FROM` lines in Dockerfiles (not currently configured); required when this pin class is introduced.

Drift first-party surfaces and provider-managed environments:

- **The `cw-build` composed image itself** — tagged with the git SHA on every build; downstream workflows pull by SHA. Internal component drift between rebuilds is captured and made reproducible by the SHA-tagged artifact, not by pinning every component inside it.
- **GitHub-hosted runners** — `ubuntu-latest`. GitHub manages runner image hardening centrally; over-pinning the runner suppresses that.
- **`.nvmrc`** — `lts/*` (forward-drifts across LTS majors).
- **Model versions** — current frontier versions, not stable-from-six-months-ago.

When reviewing, actively look for both directions:
...
- Third-party pins with no drift mechanism behind them (a `wrangler@X.Y.Z` embedded in a Dockerfile `RUN npm install` line, which dependabot cannot rewrite regardless of ecosystem config; a SHA-pinned action outside the paths dependabot's `github-actions` ecosystem covers; any pin whose location dependabot's ecosystem cannot parse or whose ecosystem is not in `.github/dependabot.yml`) — flag as a maintenance liability. The pin is only load-bearing when there is an automated bump path behind it, or when its manual-maintenance cadence is explicitly documented.
