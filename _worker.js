...
// Runs on every request: detects bot crawls, writes timestamp to KV, injects
// the latest crawl timestamp into the response HTML.
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (isBot && env.CRAWL_TRACKER) {
      ...
      ctx.waitUntil(env.CRAWL_TRACKER.put('last_crawled', now));
    }
